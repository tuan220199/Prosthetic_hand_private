...
from modeltraining import get_class
def set_cmd_cb(resp):
...
class SearchWindow(PageWindow):
    ...
    def make_handleButton(self, button, *args):
        def handleButton():
            ...
            global OFFSET_RMS, file1, FORWARD
        ...
        for j in range (8):
            try:
                ...
                if datawindow:
                    ...
                    if reg:
                        pred_class = get_class(torch.tensor(rms_.reshape(1,8)),4) 
                        print(ACTIONS[pred_class+1][0])
                        mySrc.data_signal.emit([pred_class/5+0.01] + list(mean_in_window))
                    elif OFFSET_RMS:
